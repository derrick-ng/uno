...
export default function (socket: Socket) {
  ...
  const game_id =
    location.substring(location.lastIndexOf("/") + 1) === "lobby"
      ? 0
      : parseInt(location.substring(location.lastIndexOf("/") + 1), 10);

  console.log("Hello11");
  console.log(CHAT_MESSAGE(game_id));
  console.log("Hello11");

  socket.on(
    CHAT_MESSAGE(game_id),
    ({ roomId, message, senderEmail, gravatar, timestamp }: ChatMessage) => {
